...
int main(int argc, char **argv) try {
    ...
    int c = 0;
    auto parser = ArgParser({{"a", "audio"}, {"b", "video"}, {"d", "ip"}, {"p","port"}, {"l", "local_Id"}}, {{"h", "help"}, {"v", "verbose"}});
    ...
    const string url = "ws://" + ip_address + ":" + to_string(port) + "/" + local_Id;
...
void sendInitialNalus(shared_ptr<Stream> stream, shared_ptr<ClientTrackData> video) {
    ...
    if (!initialNalus.empty()) {
        ...
        cout << "frameDuration_s: " << frameDuration_s << endl;
